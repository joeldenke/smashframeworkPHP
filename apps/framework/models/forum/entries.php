<?php	
	class EntriesModel extends ArrayIterator
	{
		...
		public function getThreads($id, $cat)
		{
			$sql  = 'SELECT * FROM '. $this->driver->quoteIdentify($this->name) .'WHERE thread_id = %d AND cat_id = %d';
			$stmt = $this->driver->prepare($sql);
			$rows = $stmt->execute($id, $cat)->fetchAll(Smash_DBA::FETCH_ASSOC);
